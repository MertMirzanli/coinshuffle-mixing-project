...
def hybrid_encrypt(public_key, plaintext: bytes) -> bytes:
    ...
    # Wire format: [4-byte length][encrypted_key][encrypted_payload]
...
def build_onion(output_address: str, recipient_public_keys: list) -> bytes:
    ...
    # Apply layers in reverse so the outermost = first recipient in the list.
...
class MixingPool:
    def __init__(self, mixing_amount: int, seed: int = None):
        ...
        # Deterministic randomness for reproducible test runs.
    ...
    def announce_phase(self):
        ...
        # Fixed shuffling order (deterministic by player_id)
        self.players.sort(key=lambda p: p.player_id)

        # In a real run this is a broadcast; here we just confirm everyone has
    ...
    # ---- Phase 2: shuffling (the heart of CoinShuffle) ------------------------
    def shuffle_phase(self):
        ...
        # STEP A: every player i builds an onion of their output address.
        # The onion is encrypted with the keys of players i+1, i+2, ..., n-1.
        # The OUTER layer is player i+1's key (first to decrypt).
        onions = []
        for i, player in enumerate(self.players):
            ...
            if future_pubkeys:
                ...
                # Last player: no one to encrypt for, send plaintext
                onion = player.output_address.encode()
            onions.append(onion)

        # STEP B: sequential pass through the players.
        # Player i receives the list from player i-1, strips their layer off
        # every item, adds their own onion, shuffles, sends to player i+1.
        current_list = []  # player 0 starts with nothing

        for i, player in enumerate(self.players):
            # Strip my layer from each ciphertext I received
            stripped = []
            ...
            # Shuffle randomly (this hides which one I added)
        ...
        # After the last player, every item is plaintext (decoded bytes)
        self.final_outputs = [
    ...
    def finalize(self) -> dict:
        ...
        # Every player signs the (input,output) set with their signing key.
...
if __name__ == "__main__":
    import sys
    # Allow command line: python coinshuffle_mixing.py [num_players] [amount] [seed]
